...
public class CheckOutPage extends BasePage {
    ...
    @FindBy(xpath = CheckOutPageObject.INPUT_FIRST_NAME_XPATH)
    WebElement inputFirstNameElement;

    // Input field for last name
    @FindBy(xpath = CheckOutPageObject.INPUT_LAST_NAME_XPATH)
    WebElement inputLastNameElement;

    // Dropdown to select country
    @FindBy(xpath = CheckOutPageObject.SELECT_COUNTRY_XPATH)
    WebElement selectCountryElement;

    // Input field for street address
    @FindBy(xpath = CheckOutPageObject.INPUT_STREET_ADDRESS_XPATH)
    WebElement inputStreetAddressElement;

    // Input field for postal code
    @FindBy(xpath = CheckOutPageObject.INPUT_POSTAL_CODE_XPATH)
    WebElement inputPostalCodeElement;

    // Input field for town or city
    @FindBy(xpath = CheckOutPageObject.INPUT_TOWN_CITY_XPATH)
    WebElement inputTownCityElement;

    // Input field for province
    @FindBy(xpath = CheckOutPageObject.INPUT_PROVINCE_XPATH)
    WebElement inputProvinceElement;

    // Input field for phone number
    @FindBy(xpath = CheckOutPageObject.INPUT_PHONE_NUMBER_XPATH)
    WebElement inputPhoneNumberElement;

    // Input field for email address
    @FindBy(xpath = CheckOutPageObject.INPUT_EMAIL_ADDRESS_XPATH)
    WebElement inputEmailAddressElement;

    // Checkbox to agree to terms and conditions
    @FindBy(xpath = CheckOutPageObject.CHK_TERMS_AND_CONDITION_XPATH)
    WebElement chkTermsAndCondition;

    // Link to select cash on delivery payment option
    @FindBy(xpath = CheckOutPageObject.LINK_CASH_ON_DELIEVERY_XPATH)
    WebElement linkCashOnDelievery;

    // Link to place the order
    @FindBy(xpath = CheckOutPageObject.LINK_PLACE_ORDER_XPATH)
